...
export default function NoticeWriteFormPageComponent () {
    ...
    const onSubmitWrite=(e)=>{
        ...
        formData.append('bbsId', signin.user_id);
        formData.append('subject', signin.제목);
        formData.append('content', signin.내용);


        axios({
            url:'https://moonjong.shop/bbs/writeAction_react.jsp',
            method:'POST',
            data: formData
        //     ,
        //     params: {
        ...
        .then((res)=>{   
            if(res.status===200 && res.data === 1 ){
                confirmModalOpen('글이 등록 되었습니다.');
                navigate('/notice');
            }
            else{
               confirmModalOpen('등록실패 데이터 확인하고 다시 시도하세요');
            }
        })
        .catch((err)=>{
