...
export default function NoticeUpdateFormPageComponent () {
    ...
    const onClickSave = async (e) => {
        ...
        formData.append("bbsId", state.번호);
        formData.append("subject", state.제목);
        formData.append("content", state.내용);

        axios({
             url:'https://moonjong.shop/bbs/updateActionReact.jsp',
             method:'POST',
             data: formData
            //  ,
            //  params: {
         ...
         .then((res)=>{            
             if(res.status===200 && res.data !== -1 ){
                confirmModalOpen('수정 되었습니다.');
                navigate('/notice');
             }
             else{
                confirmModalOpen('수정실패 데이터 확인하고 다시 시도하세요');
             }
         })
         .catch((err)=>{
             console.log('AXIOS 실패!' + err );
             confirmModalOpen('AXIOS 실패!');
         });
